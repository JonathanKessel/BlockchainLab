...
def extract_hsv_info(splash_frame, dict_all_bounding_boxes, pct1, pct2):
    ...
    lower_range_diver = np.array([0, 0, 0])
    upper_range_diver = np.array([80, 255, 255])
...
out_stack = cv.VideoWriter('./Ergebnisse/154_stack.mp4', cv.VideoWriter_fourcc('m', 'p', '4', 'v'), 10.0, (1920, 480))
while True:
    ...
    if "diver" in boxes_dict:
        ...
        # so when multiplied with original image removes all non-blue regions
        result_diver = cv.bitwise_and(diver_extract_rgb, diver_extract_rgb, mask=mask_diver)
        "Color Filter Diver --end--"

        "Getting contours and removing areas that are too small "
        # getting contours
        ret, thresh = cv.threshold(mask_diver, 127, 255, 0)
        ...
        cv.drawContours(result_diver.copy(), contours, -1, (0, 255, 0), 1)

        result_diver_hsv = cv.cvtColor(result_diver, cv.COLOR_BGR2HSV)
        # focusing on contours that are white
        intensities = []
        # for each list of contour points...
        # 255 -> black; 127 white
        for i in range(len(contours)):
            cv.drawContours(result_diver, contours, -1, (0, 255, 0), 1)
            # focusing on contours that are white
            # we are only interested in contours large enough
            # Create a mask image that contains the contour filled in
            cimg = np.zeros_like(mask_diver)
            ...
            if n_pxs < 150:
                ...
                cv.drawContours(mask_diver, contours, i, color=0, thickness=-1)
                cv.drawContours(result_diver, contours, i, color=0, thickness=-1)
            else:
                ...
                # frame_count ==  103 interesting!

                # getting mean intensity
                #for cnt in intensities:
                    #mean0 = np.mean(cnt, axis=0)
                #if int(mean0[0]) < 4:
                    # print(int(mean0[0]), int(mean0[1]), int(mean0[2]))
                    #cv.drawContours(mask_diver, contours, i, color=0, thickness=-1)
                    #cv.drawContours(result_diver, contours, i, (0, 255, 0), -1)
        ...
        black_result[ymin:ymax, xmin:xmax] = result_diver
        ...
        black_mask[ymin:ymax, xmin:xmax] = mask_diver
    ...
    if splash:
        ...
        # presentation splash supervised info
        if False:
            stack = cv.hconcat([mask_diver_no_MOG2, main_result, frame])
            cv.imwrite('./Ergebnisse/Splash/frame.jpg', frame)
            cv.imwrite('./Ergebnisse/Splash/mask.jpg', mask_diver_no_MOG2)
            cv. imwrite('./Ergebnisse/Splash/result.jpg', main_result)
    ...
    print(stack.shape)
    if frame_count == 142:
        cv.imwrite('./Ergebnisse/142_main_result_2b.jpg', main_result)
        cv.imwrite('./Ergebnisse/142_mask_no_mog2_2b.jpg', mask_diver_no_MOG2)
