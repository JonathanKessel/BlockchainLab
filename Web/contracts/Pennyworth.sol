...
contract PennyWorth{
    ...
    */
    ...
    // (price * zeit_diff * prozentsatz / 365 Tage)*100
    function patronageOwed() public view returns (uint256 patronageDue) {
        return (price.mul(now.sub(timeLastCollected)).mul(patronageNumerator).div(365 days)).div(100);
    }
    ...
    function patronageOwedRange(uint256 _time) public view returns (uint256 patronageDue) {
        return (price.mul(_time).mul(patronageNumerator).div(365 days)).div(100);
    }
    ...
    // returns the patron (of the artwork)

    function getPatronAdress() public view returns(address patron){
        return art.ownerOf(tokenID);
    }
